...
class WCFMu_Orders_Manage_Controller {
	...
	public function processing() {
	  ...
	  if(isset($wcfm_orders_manage_form_data['associate_product']) && !empty($wcfm_orders_manage_form_data['associate_product'])) {
	  	
	  	$new_order_id = '';
	  	
	  	$customer_id   = absint( $wcfm_orders_manage_form_data['customer_id'] );
	  	$product_id    = absint( $wcfm_orders_manage_form_data['associate_product'] );
	  	$variation_id  = isset( $wcfm_orders_manage_form_data['associate_product_variation'] ) ? absint( $wcfm_orders_manage_form_data['associate_product_variation'] ) : '';
	  	$quantity      = absint( $wcfm_orders_manage_form_data['product_quantity'] );
	  	...
	  	try {
				...
				if( wc_coupons_enabled() && apply_filters( 'wcfm_orders_manage_discount', true ) && isset( $wcfm_orders_manage_form_data['wcfm_om_discount'] ) && !empty( $wcfm_orders_manage_form_data['wcfm_om_discount'] ) ) {
					...
					$coupon_title = __( 'Discount', 'wc-frontend-manager-ultimate' ) . ' #' . $order->get_id() . ' (' . date( wc_date_format() . ' ' . wc_time_format(), current_time( 'timestamp' ) ) . ')';
					$new_coupon = array(
