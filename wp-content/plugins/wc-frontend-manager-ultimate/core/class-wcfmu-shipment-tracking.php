...
class WCFMu_Shipment_Tracking {
	...
	function wcfm_wcfmmarketplace_order_mark_shipped() {
      ...
      if( $tracking_code && $tracking_url ) {
				if( wcfm_is_vendor() ) {
					...
					foreach( $product_ids as $product_id ) {
						$comment_id = $order->add_order_note( sprintf( __( 'Product <b>%s</b> has been shipped to customer.<br/>Tracking Code : %s <br/>Tracking URL : <a href="%s">%s</a>', 'wc-frontend-manager-ultimate' ), get_the_title( $product_id ), $tracking_code, $tracking_url, $tracking_url ), apply_filters( 'wcfm_is_allow_shipment_note_to_customer', '1' ) );
					}
				}
			}
  ...
  public function wcfmu_shipping_tracking_orders_actions( $actions, $user_id, $order ) {
  	...
  	if( !$order->needs_shipping_address() ) return $actions;
  	...
  	$order_status = sanitize_title( $order->get_status() );
		...
		if ( $needs_shipping ) {
			$actions .= '<a class="wcfm-action-icon" href="' . get_wcfm_view_order_url($order->get_id()) . '#sm_order_shipment_options"><span class="fa fa-truck text_tip" data-tip="' . esc_attr__( 'Mark Shipped', 'wc-frontend-manager-ultimate' ) . '"></span></a>';
		} 
  ...
  public function wcfmu_wcmarketplace_shipping_tracking_orders_actions( $actions, $user_id, $order, $the_order ) {
  	...
  	// Virtual Order Handling
  	if( !$the_order->needs_shipping_address() ) return $actions;
