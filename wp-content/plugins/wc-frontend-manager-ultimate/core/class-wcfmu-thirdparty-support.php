  ...
  function wcfm_wc_box_office_product_manage_fields( ) {
	...
	function wcfm_wcrental_product_inventory_manage( $stock_fields, $product_id, $product_type ) {
		...
		foreach( $inventory_taxonomies as $inventory_taxonomy ) {
			...
			$inventory_taxonomie_elements[$inventory_taxonomy] = array( 'label' => ucfirst( str_replace( '_', ' ', str_replace( 'rnb_', '', $inventory_taxonomy ) ) ), 'custom_attributes' => array( 'placeholder' => __('Set', 'redq-rental') . ' ' . $inventory_taxonomy ), 'attributes' => array( 'multiple' => 'multiple', 'style' => 'width: 60%;' ), 'type' => 'select', 'options' => $inventory_taxonomy_options, 'class' => 'wcfm-select wcfm_ele redq_rental', 'label_class' => 'wcfm_title redq_rental' );
		}
