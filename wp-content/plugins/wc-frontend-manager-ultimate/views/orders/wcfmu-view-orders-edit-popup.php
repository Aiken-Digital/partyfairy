	...
	<form action="" method="post" id="wcfm_order_edit_form" class="order_edit-form wcfm_popup_wrapper" novalidate="">
		...
		<?php foreach( $line_items as $order_item_id => $item ) { ?>
			<p class="wcfm-order_edit-form-request-amount wcfm_popup_label">
				<label for="wcfm_order_edit_request_amount"><?php echo $item->get_name() . ' ' . __( 'Quantity', 'wc-frontend-manager-ultimate' ); ?> <span class="required">*</span></label> 
			</p>
			<?php $WCFM->wcfm_fields->wcfm_generate_form_field( array( "wcfm_order_edit_quantity" => array( 'type' => 'number', 'name' => 'wcfm_order_edit_quantity['.$order_item_id.']', 'attributes' => array( 'min' => '1', 'step' => '1' ), 'class' => 'wcfm-text wcfm_ele wcfm-order_edit-form-quantity wcfm_popup_input', 'label_class' => 'wcfm_title', 'value' => ( $item->get_quantity() ? esc_html( $item->get_quantity() ) : '1' ) ) ) ); ?>
		<?php } ?>
		
		<?php if( wc_coupons_enabled() && apply_filters( 'wcfm_orders_manage_discount', true ) ) { ?>
			<p class="wcfm-order_edit-form-request-amount wcfm_popup_label">
				<label for="wcfm_order_edit_discount_amount"><?php _e( 'Apply Discount', 'wc-frontend-manager-ultimate' ); ?></label> 
			</p>
			<?php $WCFM->wcfm_fields->wcfm_generate_form_field( array( "wcfm_om_discount" => array( 'type' => 'number', 'attributes' => array( 'min' => '1', 'step' => '1' ), 'class' => 'wcfm-text wcfm_ele wcfm-order_edit-form-discount-amount wcfm_popup_input', 'label_class' => 'wcfm_title', 'value' => '' ) ) ); ?>
		<?php } ?>
		
		<p class="wcfm-order_edit-form-reason wcfm_popup_label">
			<label for="comment"><?php _e( 'Note to Customer', 'wc-frontend-manager-ultimate' ); ?></label>
		</p>
