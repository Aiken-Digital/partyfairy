	...
	<div class="wcfm-collapse-content">
	  ...
	  <div class="wcfm-container wcfm-top-element-container">
			...
			if( $allow_wp_admin_view = apply_filters( 'wcfm_allow_wp_admin_view', true ) ) {
				?>
				<a target="_blank" class="wcfm_wp_admin_view text_tip" href="<?php echo admin_url('edit.php?post_type=shop_order'); ?>" data-tip="<?php _e( 'WP Admin View', 'wc-frontend-manager-ultimate' ); ?>"><span class="fa fab fa-wordpress fa-wordpress-simple"></span></a>
				<?php
			}
			
			if( apply_filters( 'wcfm_is_allow_orders', true ) ) {
				echo '<a class="add_new_wcfm_ele_dashboard text_tip" href="'.get_wcfm_orders_url().'" data-tip="' . __('Orders', 'wc-frontend-manager') . '"><span class="fa fa-shopping-cart"></span></a>';
			}
			?>
				...
				<div id="wcfm_orders_manage_expander" class="wcfm-content">
					...
					<table class="form-table">
						<tbody>
							<tr valign="top">
								<th scope="row">
									<label for="customer_id" class="wcfm_title"><?php _e( 'Customer', 'wc-frontend-manager-ultimate' ); ?></label>
								</th>
								<td>
									<select name="customer_id" id="customer_id" class="wc-customer-search" data-placeholder="<?php _e( 'Guest', 'wc-frontend-manager-ultimate' ); ?>" data-allow_clear="true" style="width: 80%;">
							...
							<tr valign="top">
								<th scope="row">
									<label for="wcfm_orders_manage_product_id" class="wcfm_title"><?php _e( 'Product', 'wc-frontend-manager-ultimate' ); ?></label>
								</th>
								<td>
									<?php
									$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'wcfm_orders_manage_fields_product', array(  
																																																			"associate_product" => array( 'type' => 'select', 'attributes' => array( 'style' => 'width: 80%;' ), 'class' => 'wcfm-select wcfm_ele', 'options' => array(), 'value' => '' ),
																																																			"associate_product_variation"  => array( 'type' => 'select', 'class' => 'wcfm-select wcfm_ele wcfm_ele_hide', 'attributes' => array( 'style' => 'width: 80%;' ), 'option' => array(), 'value' => '' )
																																														)) );
									?>
							...
							<tr valign="top">
								<th scope="row">
									<label for="wcfm_orders_manage_quantity" class="wcfm_title"><?php _e( 'Quantity', 'wc-frontend-manager-ultimate' ); ?></label>
								</th>
								<td>
									<?php
									$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'wcfm_orders_manage_fields_quantity', array(  
																																																			"product_quantity"  => array( 'type' => 'number', 'class' => 'wcfm-text wcfm_ele', 'attributes' => array( 'style' => 'width: 80%;' ), 'value' => '1' )
																																														)) );
									?>
									<?php do_action( 'wcfm_orders_manage_after_products_list' ); ?>
								</td>
					...
					<div class="wcfm-container">
						<div id="wcfm_om_discount_expander" class="wcfm-content">
							...
							$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'wcfm_orders_manage_fields_discount', array(  
																																																	"wcfm_om_discount"  => array( 'label' => __('Discount Amount', 'wc-frontend-manager-ultimate')  . ' (' . get_woocommerce_currency_symbol() . ')', 'type' => 'number', 'class' => 'wcfm-text wcfm_ele', 'label_class' => 'wcfm_title wcfm_ele', 'value' => '' )
																																												)) );
							?>
