jQuery(document).ready(function($) {
	...
	if( $("#associate_product").length > 0 ) {
		$("#associate_product").select2( $wcfm_simple_product_select_args );
	}
	
	// Check is Variable Product
	$("#associate_product").on('change', function() {
			...
			jQuery.each( $wcfm_search_products_list, function( id, product ) {
				if( $selected_product == id ) {
					...
					if( !jQuery.isEmptyObject( $variations ) ) {
						...
						$('#associate_product_variation').html($variations_html).removeClass('wcfm_ele_hide');
					} else {
						$('#associate_product_variation').html($variations_html).addClass('wcfm_ele_hide');
					}
				}
			});
		} else {
  ...
  function wcfm_orders_manage_form_validate() {
		...
		var associate_product = $.trim($('#wcfm_orders_manage_form').find('#associate_product').val());
		if(associate_product.length == 0) {
			$is_valid = false;
