...
if ( ! class_exists( 'Kadence_Woomail_Settings' ) ) {
	...
	class Kadence_Woomail_Settings {
		...
		public static $advanced_shipment_tracking_email_types_mapping = array(
			'customer_delivered_order' => 'Customer Delivered Order Email',
		);
		// WC Marketplace.
...
To reset your password, visit the following address:', 'kadence-woocommerce-email-designer' ),
		...
		public static function get_email_types() {
			if ( is_null( self::$email_types ) ) {
				...
				// }
				if ( class_exists( 'zorem_woocommerce_advanced_shipment_tracking' ) && get_option( 'wc_ast_status_delivered' ) && 'wc_email' === get_option( 'wc_ast_select_email_type' ) ) {
					$types = array_merge( $types, self::$advanced_shipment_tracking_email_types_mapping );
				}
