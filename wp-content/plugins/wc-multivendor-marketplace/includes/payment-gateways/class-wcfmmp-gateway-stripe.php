...
class WCFMmp_Gateway_Stripe extends WCFMmp_Abstract_Gateway {
	...
	public function process_payment( $withdrawal_id, $vendor_id, $withdraw_amount, $withdraw_charges, $transaction_mode = 'auto', $args = array() ) {
		...
		if ( $this->validate_request() ) {
			if( $this->process_stripe_payment( $args ) ) {
	...
	private function process_stripe_payment( $args = array() ) {
		...
		try {
			...
			if ($this->debug)
				wcfmmp_log( sprintf( '#%s - Stripe payment processing success: %s', sprintf( '%06u', $this->withdrawal_id ), $transfer ), 'info' );
			return true;
		} catch (\Stripe\Error\InvalidRequest $e) {
			wcfmmp_log( sprintf( '#%s - Stripe payment processing failed: %s', sprintf( '%06u', $this->withdrawal_id ), $e->getMessage() ), 'error' );
			return false;
