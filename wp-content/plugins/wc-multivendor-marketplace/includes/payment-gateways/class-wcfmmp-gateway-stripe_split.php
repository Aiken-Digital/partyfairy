						...
						try {
							...
							if (isset($this->charge['failure_message']) && empty($this->charge['failure_message'])) {
								...
								if (isset($this->charge->id)) {
									...
									wcfmmp_log( sprintf( '#%s - vendor %s payment processing complete via %s for order %s. Amount: %s', sprintf( '%06u', $withdrawal_id ), $vendor_id, 'Stripe Split Pay', $order_id, $distribution_info['commission'] . ' ' . $order->get_currency() ), 'info' );
								}
								$card_charged = true;
							...
							if ($transfer_response) {
								if( isset( $transfer_response['status'] ) && $transfer_response['status'] ) {
									...
									foreach ($transfer_response as $message) {
										wcfmmp_log( sprintf( '#%s - vendor %s payment processing failed: %s', sprintf( '%06u', $withdrawal_id ), $vendor_id, $message['message'] ), 'error' );
									}
								}
							} else {
								wcfmmp_log( sprintf( '#%s - vendor %s payment processing failed: %s', sprintf( '%06u', $withdrawal_id ), $vendor_id, __('Something went wrong please try again later.', 'wc-multivendor-marketplace') ), 'error' );
							}
